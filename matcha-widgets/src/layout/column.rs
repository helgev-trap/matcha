...
impl ColumnWidget {
    fn calc_gap_and_offset(
        ...
        let (mut gap, mut offset) = match &self.justify_content {
            ...
            | JustifyContent::Center { gap: g } => match g {
                GrowSize::Grow(s) => {
                    if child_count >= 2 {
                        let available = container_size - total_child_height;
                        let grow_val = s.size([child_max_width, container_size], &mut rep_child_size, ctx);
                ...
                GrowSize::Fixed(s) => {
                    (s.size([child_max_width, container_size], &mut rep_child_size, ctx), 0.0)
                }
            },
    ...
    /// Compute (child_size, child_affine) for each child given `bounds`.
    fn compute_arrangements(
        ...
        let child_max_width: f32 = child_sizes.iter().map(|s| s[0]).fold(0.0_f32, f32::max);
        ...
        for (i, &child_size) in child_sizes.iter().enumerate() {
            ...
            let spacing = match &self.justify_content {
                JustifyContent::SpaceBetween => {
                    if self.children.len() > 1 && i < self.children.len() - 1 {
                        (bounds[1] - total_child_height) / (self.children.len() - 1) as f32
                    } else {
                        0.0
                    }
                }
                JustifyContent::SpaceAround => {
                    if self.children.len() > 1 {
                        (bounds[1] - total_child_height) / self.children.len() as f32
                    } else {
                        0.0
                    }
                }
            ...
            let affine = Matrix4::new_translation(&nalgebra::Vector3::new(x_offset, y_offset, 0.0));
...
impl Widget for ColumnWidget {
    ...
    fn update(&mut self, view: &Column, ctx: &UiContext) -> WidgetInteractionResult {
    ...
    fn measure(&self, constraints: &Constraints, ctx: &UiContext) -> [f32; 2] {
        ...
        if self.children.len() > 1 {
            total_height += gap * (self.children.len() - 1) as f32;
        }
