...
impl Size {
    ...
    pub fn vw(vw: f32) -> Self {
        Self {
            f: Arc::new(move |_, _, ctx: &UiContext| vw * ctx.viewport_size().map(|v| v[0]).unwrap_or(0.0)),
        }
    }
    ...
    pub fn vh(vh: f32) -> Self {
        Self {
            f: Arc::new(move |_, _, ctx: &UiContext| vh * ctx.viewport_size().map(|v| v[1]).unwrap_or(0.0)),
        }
    }
